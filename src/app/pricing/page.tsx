...
export default function PricingPage() {
  ...
  const handleDelete = async (item: any) => {
    if (!await showConfirm(t('confirm_delete', { name: item.name || item.label || item.id }), { variant: 'danger', confirmLabel: t('delete') })) return;
    try {
  ...
  const tabs = [
    { key: 'folding', label: t('tab_folding'), icon: 'bi-collection' },
    { key: 'areaRank', label: t('tab_area_rank'), icon: 'bi-map' },
    { key: 'period', label: t('tab_period'), icon: 'bi-calendar-range' },
    { key: 'size', label: t('tab_size'), icon: 'bi-aspect-ratio' },
  ] as const;
      ...
      <div className="max-w-5xl mx-auto px-4 py-10">
        ...
        <div className="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
          <div className="p-5 border-b border-slate-100 flex items-center justify-between">
            <h2 className="font-bold text-slate-700">{tabs.find(tb => tb.key === tab)?.label}</h2>
            {canCreate && (
            ...
            <table className="w-full text-sm">
              <thead className="bg-slate-50 border-b border-slate-100">
                <tr>
                  {tab === 'folding' && (
                    <>
                      <th className="px-5 py-3 text-left font-bold text-slate-600">{t('col_name')}</th>
                      <th className="px-5 py-3 text-right font-bold text-slate-600">{t('col_unit_price')}</th>
                      <th className="px-5 py-3 text-center font-bold text-slate-600">{t('col_sort_order')}</th>
                      <th className="px-5 py-3 text-center font-bold text-slate-600">{t('col_status')}</th>
                      <th className="px-5 py-3"></th>
                    </>
                  )}
                  {tab === 'areaRank' && (
                    <>
                      <th className="px-5 py-3 text-left font-bold text-slate-600">{t('col_rank_name')}</th>
                      <th className="px-5 py-3 text-right font-bold text-slate-600">{t('col_posting_price')}</th>
                      <th className="px-5 py-3 text-left font-bold text-slate-600">{t('col_description')}</th>
                      <th className="px-5 py-3"></th>
                    </>
                  )}
                  {tab === 'period' && (
                    <>
                      <th className="px-5 py-3 text-left font-bold text-slate-600">{t('col_label')}</th>
                      <th className="px-5 py-3 text-center font-bold text-slate-600">{t('col_min_days')}</th>
                      <th className="px-5 py-3 text-center font-bold text-slate-600">{t('col_max_days')}</th>
                      <th className="px-5 py-3 text-center font-bold text-slate-600">{t('col_multiplier')}</th>
                      <th className="px-5 py-3"></th>
                    </>
                  )}
                  {tab === 'size' && (
                    <>
                      <th className="px-5 py-3 text-left font-bold text-slate-600">{t('col_size_name')}</th>
                      <th className="px-5 py-3 text-right font-bold text-slate-600">{t('col_print_price')}</th>
                      <th className="px-5 py-3 text-right font-bold text-slate-600">{t('col_addon_price')}</th>
                      <th className="px-5 py-3"></th>
                    </>
                  )}
                </tr>
              ...
              <tbody className="divide-y divide-slate-100">
                ...
                {tab === 'period' && data.periodPrices.map(item => (
                  <tr key={item.id} className="hover:bg-slate-50">
                    ...
                    <td className="px-5 py-3 text-center font-mono text-slate-700">{item.maxDays ? `${item.maxDays}` : t('no_limit')}</td>
          ...
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md p-6">
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              {tab === 'folding' && (
                <>
                  <div>
                    <label className="block text-sm font-bold text-slate-700 mb-1">{t('form_name')}</label>
                    <input type="text" required value={form.name || ''} onChange={e => setForm((p: any) => ({...p, name: e.target.value}))} className="w-full border border-slate-300 p-3 rounded-xl text-sm outline-none focus:ring-2 focus:ring-indigo-500" placeholder={t('placeholder_folding_name')} />
                  </div>
                  <div>
                    <label className="block text-sm font-bold text-slate-700 mb-1">{t('form_unit_price')}</label>
                  ...
                  <div>
                    <label className="block text-sm font-bold text-slate-700 mb-1">{t('form_sort_order')}</label>
              ...
              {tab === 'areaRank' && (
                <>
                  <div>
                    <label className="block text-sm font-bold text-slate-700 mb-1">{t('form_rank_name')}</label>
                    <input type="text" required value={form.name || ''} onChange={e => setForm((p: any) => ({...p, name: e.target.value}))} className="w-full border border-slate-300 p-3 rounded-xl text-sm outline-none focus:ring-2 focus:ring-indigo-500" placeholder={t('placeholder_rank_name')} />
                  </div>
                  <div>
                    <label className="block text-sm font-bold text-slate-700 mb-1">{t('form_posting_price')}</label>
                  ...
                  <div>
                    <label className="block text-sm font-bold text-slate-700 mb-1">{t('form_description')}</label>
                    <input type="text" value={form.description || ''} onChange={e => setForm((p: any) => ({...p, description: e.target.value}))} className="w-full border border-slate-300 p-3 rounded-xl text-sm outline-none focus:ring-2 focus:ring-indigo-500" placeholder={t('placeholder_rank_desc')} />
                  </div>
                </>
              )}
              {tab === 'period' && (
                <>
                  <div>
                    <label className="block text-sm font-bold text-slate-700 mb-1">{t('form_label')}</label>
                    <input type="text" value={form.label || ''} onChange={e => setForm((p: any) => ({...p, label: e.target.value}))} className="w-full border border-slate-300 p-3 rounded-xl text-sm outline-none focus:ring-2 focus:ring-indigo-500" placeholder={t('placeholder_period_label')} />
                  </div>
                  <div className="grid grid-cols-2 gap-3">
                    <div>
                      <label className="block text-sm font-bold text-slate-700 mb-1">{t('form_min_days')}</label>
                    ...
                    <div>
                      <label className="block text-sm font-bold text-slate-700 mb-1">{t('form_max_days_label')}</label>
                      <input type="number" value={form.maxDays ?? ''} onChange={e => setForm((p: any) => ({...p, maxDays: e.target.value}))} className="w-full border border-slate-300 p-3 rounded-xl text-sm outline-none focus:ring-2 focus:ring-indigo-500" placeholder={t('form_max_days_placeholder')} />
                    </div>
                  </div>
                  <div>
                    <label className="block text-sm font-bold text-slate-700 mb-1">{t('form_multiplier')}</label>
              ...
              {tab === 'size' && (
                <>
                  <div className="bg-slate-50 border border-slate-200 rounded-xl p-3 text-sm font-bold text-slate-600">
                    <i className="bi bi-aspect-ratio mr-1"></i> {editTarget?.name}{t('form_size_setting')}
                  </div>
                  <div>
                    <label className="block text-sm font-bold text-slate-700 mb-1">{t('form_print_price')}</label>
                  ...
                  <div>
                    <label className="block text-sm font-bold text-slate-700 mb-1">{t('form_addon_price')}</label>
              ...
              <div className="flex gap-3 pt-2">
                <button type="button" onClick={() => setShowModal(null)} className="flex-1 py-2.5 font-bold text-slate-600 bg-slate-100 hover:bg-slate-200 rounded-xl text-sm">{t('cancel')}</button>
