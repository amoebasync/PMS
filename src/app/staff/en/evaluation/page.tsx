...
export default function StaffEvaluationPageEn() {
  ...
  return (
    <div className="space-y-5">
      <h1 className="text-lg font-black text-slate-800 flex items-center gap-2">
        <i className="bi bi-award-fill text-indigo-500"></i> Rating
      </h1>
      ...
      <div className="bg-white rounded-2xl border border-slate-200 shadow-sm p-5">
        <div className="flex items-center gap-5">
          ...
          <div className="flex-1">
            <p className="text-xs text-slate-500 font-bold">Current Rank</p>
            <p className="text-2xl font-black text-slate-800">{currentScore} <span className="text-sm font-bold text-slate-400">pts</span></p>
            {pointsToNext != null && nextRankName && pointsToNext > 0 && (
              <p className="text-xs text-indigo-600 font-bold mt-1">
                <i className="bi bi-arrow-up-circle-fill mr-1"></i>
                Next rank ({nextRankName}): {pointsToNext} pts away
              </p>
            )}
          </div>
        </div>
      ...
      {chartData.length > 1 && (
        <div className="bg-white rounded-2xl border border-slate-200 shadow-sm p-5">
          <h2 className="text-sm font-bold text-slate-700 mb-3">Score Trend (12 weeks)</h2>
          <ResponsiveContainer width="100%" height={200}>
            <LineChart data={chartData}>
              ...
              <Line type="monotone" dataKey="totalScore" stroke="#3b82f6" name="Total" strokeWidth={2} dot={{ r: 3 }} />
              <Line type="monotone" dataKey="performanceScore" stroke="#22c55e" name="Performance" strokeWidth={1.5} dot={{ r: 2 }} />
              <Line type="monotone" dataKey="qualityScore" stroke="#ef4444" name="Quality" strokeWidth={1.5} dot={{ r: 2 }} />
            </LineChart>
          </ResponsiveContainer>
        </div>
      )}
      ...
      <div className="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
        <div className="px-5 py-3 border-b border-slate-100">
          <h2 className="text-sm font-bold text-slate-700">Weekly Evaluations</h2>
        </div>
        {evaluations.length === 0 ? (
          <p className="text-sm text-slate-400 text-center py-8">No evaluation data available</p>
        ) : (
                  ...
                  >
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        ...
                        <p className="text-xs text-slate-400">{ev.totalScore} pts</p>
                      </div>
                    </div>
                    ...
                    <div className="px-5 pb-4 bg-slate-50">
                      <div className="grid grid-cols-2 gap-3 text-xs">
                        ...
                        <div className="bg-white rounded-xl p-3">
                          ...
                          <p className="text-lg font-bold text-slate-700">{ev.attendanceDays}</p>
                        </div>
                        <div className="bg-white rounded-xl p-3">
                          <span className="text-slate-500">Sheets Delivered</span>
                          <p className="text-lg font-bold text-slate-700">{ev.totalSheets.toLocaleString()}</p>
                        </div>
                        {ev.complaintCount > 0 && (
                          <div className="col-span-2 bg-red-50 rounded-xl p-3">
                            <span className="text-red-500">Complaints</span>
                            <p className="text-lg font-bold text-red-600">{ev.complaintCount}</p>
                          </div>
                        )}
                      </div>
                    </div>
