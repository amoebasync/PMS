    ...
    <div className="fixed inset-0 bg-black/50 z-50 flex items-end justify-center">
      <div className="bg-white w-full max-w-lg rounded-t-2xl shadow-2xl">
        ...
        <div className="px-5 pb-2">
          <h3 className="text-lg font-bold text-slate-800">Cash Receipt Confirmation</h3>
          <p className="text-xs text-slate-500 mt-1">Confirm that you have received the following amount in cash</p>
        </div>

        <div className="mx-5 bg-emerald-50 rounded-xl p-4 border border-emerald-100">
          <div className="text-xs text-emerald-600 font-bold">Amount Received</div>
        ...
        <div className="mx-5 mt-3 bg-slate-50 rounded-lg p-3 border border-slate-100">
          <p className="text-xs text-slate-600">
            ...
            I confirm that I have received the above amount in cash and the amount is correct.
          </p>
        </div>

        <div className="px-5 mt-3">
          <div className="flex items-center justify-between mb-1.5">
            <label className="text-xs font-bold text-slate-600">Signature</label>
            <button onClick={clearCanvas} className="text-[10px] text-slate-400 hover:text-red-500 transition-colors">
              <i className="bi bi-arrow-counterclockwise mr-0.5"></i>Clear
            </button>
          </div>
          ...
          <p className="text-[10px] text-slate-400 mt-1 text-center">Please sign in the box above</p>
        </div>

        <div className="flex gap-3 px-5 py-4 mt-2">
          <button onClick={onCancel} className="flex-1 py-3 rounded-xl border border-slate-200 text-slate-600 text-sm font-bold hover:bg-slate-50 transition-colors">
            Cancel
          </button>
          <button onClick={handleSave} disabled={!hasDrawn}
            ...
            <i className="bi bi-check-lg mr-1"></i>Confirm Receipt
          </button>
        </div>
      </div>
    </div>
                  ...
                  <div className="flex flex-col items-end gap-1">
                    ...
                    {isConfirmed && (
                      <span className="text-[10px] font-bold px-2 py-0.5 rounded-full bg-blue-100 text-blue-700">
                        <i className="bi bi-check-circle-fill mr-0.5"></i>Received
                      </span>
                    )}
                  </div>
                    ...
                    {isConfirmed && (
                      <div className="mt-3 pt-3 border-t border-slate-100">
                        <p className="text-[10px] text-slate-400">
                          ...
                          Confirmed at: {new Date(record.cashReceivedAt!).toLocaleString('en-US', { timeZone: 'Asia/Tokyo' })}
                        </p>
                      </div>
                    )}
                  </div>
      ...
      {submitting && (
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center">
          <div className="bg-white rounded-2xl px-8 py-6 shadow-2xl text-center">
            ...
            <p className="text-sm text-slate-600 mt-3">Submitting...</p>
          </div>
        </div>
      )}
    </div>
