    ...
    <div className="space-y-5">
      ...
      {pendingAssignment && (
        <div className="bg-indigo-600 rounded-2xl p-5 text-white shadow-md">
          <div className="flex items-start gap-3 mb-4">
            ...
            <div>
              <p className="font-black text-base leading-tight">A training test is waiting for you</p>
              <p className="text-indigo-200 text-xs mt-1">
                Assigned on{' '}
                {new Date(pendingAssignment.assignedAt).toLocaleDateString('en-US', {
                  timeZone: 'Asia/Tokyo',
                  year: 'numeric',
                  month: 'long',
                  day: 'numeric',
      ...
      {isTrainingTestPassed && !pendingAssignment && (
        <div className="bg-emerald-50 border border-emerald-200 rounded-2xl p-5 flex items-center gap-3">
          ...
          <div>
            <p className="font-black text-emerald-700 text-base">Training Test Passed</p>
            <p className="text-emerald-600 text-xs mt-0.5">Congratulations!</p>
          </div>
        </div>
      )}
      ...
      {!pendingAssignment && !isTrainingTestPassed && (
        <div className="bg-white rounded-2xl border border-slate-100 p-5 text-center">
          ...
          <p className="text-sm text-slate-400">No test assigned at the moment</p>
        </div>
      )}
      ...
      {pastResults.length > 0 && (
        <div>
          <h2 className="text-sm font-bold text-slate-600 mb-3 px-1">Past Results</h2>
          <div className="space-y-2">
            {pastResults.map((r) => (
              ...
              >
                <div
                  className={`w-10 h-10 rounded-full flex items-center justify-center shrink-0 font-black text-xs ${
                ...
                <span className="text-xs text-slate-400">#{r.attemptNumber}</span>
              </div>
            ))}
          </div>
        </div>
    ...
    <div className="flex flex-col gap-4">
      ...
      {currentQuestion.imageUrl && (
        ...
        >
          ...
          />
          <p className="text-xs text-slate-400 text-center mt-1">Tap to zoom</p>
        </div>
      )}
  ...
  return (
    <div className="space-y-5">
      ...
      {localAnswers.length > 0 && (
        <div>
          <h2 className="text-sm font-bold text-slate-600 mb-3 px-1">Answer Summary</h2>
            ...
            {localAnswers.map((a, i) => (
              <div key={a.questionId} className="flex items-start gap-3 p-4">
                ...
                <div className="flex-1 min-w-0">
                  <p className="text-xs text-slate-400 mb-0.5">Question {i + 1}</p>
...
// ===== Main Page =====
export default function StaffTestPageEn() {
