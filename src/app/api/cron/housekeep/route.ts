...
// デフォルト保持期間（日数）— SystemSetting テーブルから上書き可能
const DEFAULT_RETENTION_DAYS = {
  ...
  inspectionGps: 365,
  passwordTokens: 7,
  pickingVerifications: 365,
  interviewSlots: 730,
  trainingSlots: 730,
  sessionEvents: 365,
};

// SystemSetting キー → RETENTION_DAYS フィールド のマッピング
const SETTING_KEY_MAP: Record<string, keyof typeof DEFAULT_RETENTION_DAYS> = {
  hkNotifications: 'notifications',
  hkAuditLogs: 'auditLogs',
  hkGpsPoints: 'gpsPoints',
  hkInspectionGps: 'inspectionGps',
  hkPasswordTokens: 'passwordTokens',
  hkPickingVerifications: 'pickingVerifications',
  hkInterviewSlots: 'interviewSlots',
  hkTrainingSlots: 'trainingSlots',
  hkSessionEvents: 'sessionEvents',
};

/** SystemSetting から保持期間を取得（未設定ならデフォルト値） */
async function getRetentionDays() {
  const retention = { ...DEFAULT_RETENTION_DAYS };
  try {
    const settings = await prisma.systemSetting.findMany({
      where: { key: { in: Object.keys(SETTING_KEY_MAP) } },
    });
    for (const s of settings) {
      const field = SETTING_KEY_MAP[s.key];
      if (field) {
        const val = parseInt(s.value, 10);
        if (!isNaN(val) && val > 0) retention[field] = val;
      }
    }
  } catch (err) {
    console.error('[Housekeep] SystemSetting読み込みエラー、デフォルト値を使用:', err);
  }
  return retention;
}
...
export async function GET(request: Request) {
  ...
  // SystemSettingから保持期間を取得
  const RETENTION_DAYS = await getRetentionDays();
  results.retentionDays = RETENTION_DAYS;
  console.log('[Housekeep] 保持期間:', JSON.stringify(RETENTION_DAYS));
  ...
  // ─────────────────────────────────────────────
  try {
    const cutoff = new Date(Date.now() - RETENTION_DAYS.sessionEvents * 24 * 60 * 60 * 1000);
