...
export async function POST(
  ...
  try {
    ...
    if (inspection.feedbackTaskId) {
      return NextResponse.json({ error: 'フィードバックタスクは既に作成されています' }, { status: 400 });
    }
    ...
    const result = await prisma.$transaction(async (tx) => {
      const task = await tx.task.create({
        data: {
          title: `[フィードバック] ${distributorName} - ${areaName}`,
          description: note,
          priority: 'MEDIUM',
          status: 'PENDING',
          dueDate: taskDueDate,
    ...
    console.error('Feedback task creation error:', err);
