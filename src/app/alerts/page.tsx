...
export default function AlertsPage() {
  ...
  return (
    <div className="space-y-5 animate-in fade-in duration-300 max-w-7xl mx-auto pb-10">
            ...
            {[{ key: 'ALL', label: t('severity_all') }, { key: 'CRITICAL', label: t('severity_critical') }, { key: 'WARNING', label: t('severity_warning') }, { key: 'INFO', label: t('severity_info') }].map(s => (
              <button
                ...
                className={`px-3 py-1.5 rounded-lg text-xs font-bold transition-all ${
                  severityFilter === s.key ? 'bg-white text-indigo-700 shadow-sm' : 'text-gray-500 hover:text-gray-700'
                }`}
              >
                {s.label}
              </button>
              ...
              alerts.map(alert => {
                ...
                return (
                  <tr key={alert.id} className="hover:bg-gray-50/50">
                    ...
                    <td className="px-5 py-3">
                      <span className={`inline-flex items-center gap-1.5 text-xs font-bold px-2.5 py-1 rounded-lg ${alert.category.colorCls || 'bg-gray-100 text-gray-600'}`}>
                    ...
                    <td className="px-5 py-3">
                      ...
                      {alert.message && <div className="text-xs text-gray-400 mt-0.5 truncate max-w-md">{alert.message}</div>}
                    </td>
      ...
      {/* Resolve modal */}
      {resolveTarget && (
        <div className="fixed inset-0 bg-black/40 backdrop-blur-sm z-[2000] flex items-center justify-center p-4" onClick={() => setResolveTarget(null)}>
            ...
            <div className="p-6 space-y-4">
              ...
              <div className="bg-gray-50 rounded-xl p-4 space-y-2">
                <div className="flex items-center gap-2">
                  ...
                  <span className={`inline-flex items-center gap-1 text-xs font-bold px-2 py-0.5 rounded ${resolveTarget.category.colorCls || 'bg-gray-100 text-gray-600'}`}>
                    {resolveTarget.category.icon && <i className={`bi ${resolveTarget.category.icon}`} />}
                    {resolveTarget.category.name}
                  </span>
                </div>
                <h4 className="font-bold text-gray-800">{resolveTarget.title}</h4>
                {resolveTarget.message && <p className="text-sm text-gray-500">{resolveTarget.message}</p>}
                <div className="text-xs text-gray-400">{formatDate(resolveTarget.createdAt)}</div>
